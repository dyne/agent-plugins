...
/**
 * MCP initialization guidance is delivered once per connection instead of
 * duplicating the routing policy in host startup hooks and tool descriptions.
 * Keep the first 512 bytes independently actionable for MCP clients that
 * truncate server instructions.
 */
export const MCP_SERVER_INSTRUCTIONS =
  "For large inspection, test/log output, filtering, or data processing, use ctx_batch_execute or ctx_execute_file and report only derived findings. Use ctx_search for indexed follow-up. Keep file writes on native edit tools; read governing AGENTS.md and SKILL.md with the native reader.";
...
/** MCP-visible selection cues; detailed recipes live in the selected skill. */
export const COMPACT_TOOL_DESCRIPTIONS: Record<string, string> = {
  ctx_execute: "Run code in a sandbox. WHEN: filter, parse, compare, or summarize uncertain/large output. Use native tools for short fixed observations and writes. RETURNS: only printed derived findings; large intentional output is indexed. EXAMPLE: summarize test failures.",
  ctx_execute_file: "Run code over one file as FILE_CONTENT in a sandbox. WHEN: derive facts from a large/structured file without reading it verbatim. Use native Read before editing. RETURNS: only printed findings. EXAMPLE: count ERROR lines.",
  ctx_index: "Store content for later ctx_search retrieval. WHEN: durable docs, references, or instructions need precise recall. Do not use for one-off logs; process those in a sandbox. RETURNS: indexing metadata. EXAMPLE: index an API guide.",
  ctx_search: "Search indexed content and session memory. WHEN: recall prior decisions, indexed sources, or timeline context; batch related queries. Capture unseen data first. RETURNS: ranked snippets. EXAMPLE: search decisions with sort: timeline.",
...
export const MCP_TOOL_DESCRIPTION_BUDGET_BYTES = 3_000;
...
(server as unknown as { registerTool: (...args: unknown[]) => unknown }).registerTool = (...args: unknown[]) => {
  ...
  const compactDescription = COMPACT_TOOL_DESCRIPTIONS[name];
  const visibleConfig = compactDescription
    ? { ...config, description: compactDescription }
    : config;
  REGISTERED_CTX_TOOLS.push({ name, config: visibleConfig, handler: wrappedHandler });
  args[1] = visibleConfig;
...
function wrapToolHandler(
  ...
  return async (toolArgs: Record<string, unknown>) => {
    ...
    noteRequestStart();
    try {
      const codexContext = resolveCodexToolContext();
      return codexContext
        ? await withProjectDirOverride(codexContext, async () => handler(toolArgs))
...
/** Resolve once at the MCP tool boundary; never let a Codex handler consult a sibling session. */
export function resolveCodexToolContext(): ToolContextOverride | undefined {
  try {
    if (detectPlatform().platform !== "codex") return undefined;
  } catch {
    return undefined;
  }
  const sessionId = process.env.CODEX_THREAD_ID;
  if (!sessionId) return undefined;
  return { projectDir: getProjectDir(), sessionId };
}
 ...
 *
 * Claude Code may explicitly allow an external read through its native
 * `permissions.allow` rules. Codex has no equivalent JSON permission file, so
 * its data-plane reads stay confined to the invocation workspace.
 *
...
function checkProjectBoundary(
  ...
  try {
    const projectDir = getProjectDir();
    const isCodex = detectPlatform().platform === "codex";
    const allowGlobs = isCodex ? [] : readToolPermissionPatterns("Read", "allow", projectDir);
    ...
    return trackResponse(toolName, {
      content: [{
        type: "text" as const,
        text: isCodex
          ? `File access blocked: "${filePath}" resolves outside the authorized workspace ` +
            `(${projectDir}). Use Codex's native read surface or copy an explicitly intended artifact into the workspace.`
          :
